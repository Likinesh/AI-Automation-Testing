...
export async function POST(req: NextRequest) {
  try {
    ...
    // Generate script using Gemini if forced, or if no script is cached
    if (forceRegenerate) {
...
`;
      ...
      generatedCode = generatedCode.replace(/^```(javascript|js|typescript|ts)?\s*/i, "");
      ...
      // Mark database status as running
      await db
        .update(TestCasesTable)
        .set({ status: "running" })
        .where(eq(TestCasesTable.id, testCase.id));
